...
function processForm(e) {
    ...
    if (e.preventDefault)
        e.preventDefault(); // Per evitare che il form provi a fare cose che non deve fare
    ...
    if (bindec)
        numeroconvertito = conversionamento(parseInt(document.getElementById("numero").value), 1); //  chiama la funzione conversionamento dando un numero e 0
    if (decbin)
        numeroconvertito = conversionamento(parseInt(document.getElementById("numero").value), 0); // se l'utente ha selezionato decbin e 1 se l'utente ha selezionato bindec
